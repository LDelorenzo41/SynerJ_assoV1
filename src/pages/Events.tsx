...
const LogoDisplay: React.FC<LogoDisplayProps> = ({ src, alt, size = 'w-8 h-8', fallbackIcon: FallbackIcon, iconColor = 'text-encre-3 dark:text-slate-500' }) => {
...
Style visuel : illustration vectorielle moderne, design plat (flat design), couleurs vives et harmonieuses, composition minimaliste et épurée, formes géométriques claires, sensation de dynamisme et d'énergie.`;
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-encre">Événements</h1>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-terracotta-soft dark:bg-terracotta-soft border border-terracotta dark:border-terracotta p-4 rounded-lg">
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <Calendar className="h-5 w-5 text-terracotta dark:text-terracotta" />
          </div>
          <div className="ml-3">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-papier rounded-ds-md border border-[var(--ds-border)] shadow-ds-sm p-6 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-[var(--ds-border-2)] dark:border-[var(--ds-border-2)] rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent bg-papier border border-[var(--ds-border-2)] text-encre placeholder:text-encre-3"
              ...
              <div>
                ...
                <div className="relative">
                  <MapPin className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-encre-3 dark:text-slate-500" />
                  <input
                    type="text"
                    value={eventForm.location}
                    onChange={(e) => setEventForm({ ...eventForm, location: e.target.value })}
                    className="w-full pl-10 pr-3 py-2 border border-[var(--ds-border-2)] dark:border-[var(--ds-border-2)] rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent bg-papier border border-[var(--ds-border-2)] text-encre placeholder:text-encre-3"
              ...
              <div>
                ...
                <div className="flex space-x-3">
                  <label className="flex-1">
                    ...
                    />
                    <div className="w-full p-3 border-2 border-dashed border-[var(--ds-border-2)] dark:border-[var(--ds-border-2)] rounded-lg text-center cursor-pointer hover:border-terracotta dark:hover:border-terracotta hover:bg-terracotta-soft dark:hover:bg-terracotta-soft transition-colors">
      ...
      <div className="bg-papier rounded-ds-md border border-[var(--ds-border)] shadow-ds-sm shadow-sm rounded-lg overflow-hidden">
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-600">
          {events.map((event) => {
            ...
            return (
              <div key={event.id} className="px-6 py-6 hover:bg-papier-2">
                <div className="flex items-center space-x-3 mb-4 pb-3 border-b border-[var(--ds-border)] dark:border-[var(--ds-border-2)]">
                  ...
                  />
                  <div>
                    <h4 className="text-sm font-medium text-encre">{event.clubs.name}</h4>
                    <div className="flex items-center space-x-2 mt-1">
                      {event.visibility === 'Public' ? (
                        <Eye className="h-3 w-3 text-ds-success dark:text-ds-success" />
                      ) : (
                        <EyeOff className="h-3 w-3 text-ds-warning dark:text-ds-warning" />
                      )}
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xl font-bold text-encre mb-3">{event.name}</h3>
                        ...
                        {event.description && (
                          <div className="mb-4">
                            <p className={`text-encre ${!isExpanded ? 'line-clamp-2' : ''}`}>
                              {event.description}
                            </p>
                            {event.description.length > 150 && (
                              <button
                                onClick={() => toggleEventExpansion(event.id)}
                                className="mt-2 text-sm text-terracotta dark:text-terracotta hover:text-terracotta-deep dark:hover:text-terracotta flex items-center"
                              >
                                {isExpanded ? (
                                  <>
  ...
  />
                            ...
                            <button
                              ...
                              className={`flex items-center space-x-1 px-3 py-1 rounded-lg text-sm transition-colors ${
                                userCalendarEvents.includes(event.id)
                                  ...
                                  : 'bg-ds-info-soft dark:bg-ds-info-soft/30 text-ds-info dark:text-ds-info hover:bg-ds-info-soft dark:hover:bg-ds-info-soft/50'
                              } disabled:opacity-50`}
                              title={
                                userCalendarEvents.includes(event.id) 
                ...
                <div className="mt-6 border-t border-[var(--ds-border)] dark:border-[var(--ds-border-2)] pt-6">
                  <CommentSection 
                    eventId={event.id} 
          ...
          {events.length === 0 && (
            <div className="px-6 py-12 text-center">
              <Calendar className="mx-auto h-12 w-12 text-encre-3 dark:text-slate-500 mb-4" />
